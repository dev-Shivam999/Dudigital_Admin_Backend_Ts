    ...
    const renderMemberCard = (member) => (
        <div key={member._id} style={{
            ...
            <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: '0.2rem 0', color: '#666', fontSize: '0.9rem' }}>{member.designation}</p>
                <span style={{ fontSize: '0.8rem', backgroundColor: '#e9ecef', padding: '0.2rem 0.5rem', borderRadius: '4px' }}>
                    {member.category || 'Uncategorized'}
                </span>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button onClick={() => handleEdit(member)} style={btnEdit}>Edit</button>
                <button onClick={() => handleDelete(member._id)} style={btnDelete}>Delete</button>
            </div>
        </div>
    );

    const btnEdit = { padding: '5px 10px', backgroundColor: '#ffc107', border: 'none', borderRadius: '4px', cursor: 'pointer' };
    const btnDelete = { padding: '5px 10px', backgroundColor: '#dc3545', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' };
        ...
        <div style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto' }}>
                    ...
                    {isGrouped ? (
                        Object.keys(teamData).map(category => (
                            <div key={category} style={{ marginBottom: '2rem' }}>
                                ...
                                <div>
                                    {teamData[category].map(renderMemberCard)}
                                </div>
                            </div>
                        ))
                    ) : (
            ...
            {showModal && (
                <div style={{
                    ...
                    <div style={{ backgroundColor: '#fff', padding: '2rem', borderRadius: '8px', width: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
                                <button type="button" onClick={() => setShowModal(false)} style={{ padding: '0.5rem 1rem', cursor: 'pointer' }}>Cancel</button>
                                <button type="submit" style={{ padding: '0.5rem 1rem', backgroundColor: '#28a745', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Save</button>
                            </div>
                        </form>
                    </div>
                </div>
